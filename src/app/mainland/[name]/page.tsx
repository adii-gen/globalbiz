...
//         // SAME fallback array like your original code
...
//         // Select fallback for current index
...
export default function MainlandPage() {
  ...
  const getDetail = useDetailsCache(state => state.getDetail);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const detailData = await getDetail(mainlandSlug);
        setData(detailData);
      } catch (err) {
        console.error("Error loading mainland details:", err);
        setError(
  ...
  }, [mainlandSlug, getDetail]);

  const mainlandData = data;
    ...
    <div className="flex justify-center space-x-2">
      ...
      {!loading && mainlandData && (
        <div className="space-y-6">
          ...
          {/* Benefits Section */}
          {mainlandData?.benefits && mainlandData.benefits.length > 0 && (
            <section
              className="relative bg-cover bg-center bg-no-repeat px-4 md:px-8 lg:px-18 xl:px-24 py-16"
              style={{ backgroundImage: "url('/images/business-type-bg.jpg')" }}
            >
          ...
          {/* License Types Section */}
          {mainlandData?.licenseTypes && mainlandData.licenseTypes.length > 0 && (
            <section className="bg-gray-50 px-4 md:px-8 lg:px-16 xl:px-24 py-16">
              {/* Title */}
              <div className="text-center mb-12">
                <h2 className="text-3xl md:text-4xl font-bold text-blue mb-2 font-oswald tracking-wide">
              ...
              {/* License Cards Grid */}
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-8xl mx-auto">
                {mainlandData.licenseTypes.map((license: LicenseType, index: number) => {
                  const fallbackImages = [
                    "/licenses/GeneralLicense.png",
                    "/licenses/IndustrialLicense.png",
                    "/licenses/PremiumConsultancy.png",
          ...
          {/* Business Process Section */}
          <ProcessCards 
            processes={mainlandData?.buesinessProcess || []} 
            mainlandName={mainlandData?.name} 
          />
