...
// import { DetailData, Perk, BusinessProcess } from "@/types/details";
...
export default function OffshorePage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const detailData = await getDetail(offshoreSlug);
        setData(detailData);
      } catch (err) {
        console.error("Error loading offshore details:", err);
        setError(
  ...
  }, [offshoreSlug, getDetail]);

  const offshoreData = data;
    ...
    <div className="flex justify-center space-x-2">
      ...
      {!loading && offshoreData && (
        <div className="space-y-6">
          ...
          {offshoreData?.buesinessProcess && offshoreData.buesinessProcess.length > 0 && (
            <OffshoreProcessCards 
